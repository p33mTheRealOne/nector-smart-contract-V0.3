...
async function scan(program: anchor.Program) {
  ...
  const sellerFundedOrders =
    orders.filter((o) => o.account.state === 2);
  ...
  for (const order of sellerFundedOrders) {
    ...
    const deadline =
      fundedAt + acc.shippingHours * 3600;
    ...
    console.log("Trigger timeout:", orderPda.toBase58());
